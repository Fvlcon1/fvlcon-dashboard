...
const FaceTracking = ({
  ...
  videoElement: HTMLVideoElement, canvasContainerID : string
}) => {
    const faceTracking = async (videoElement: HTMLVideoElement, canvasContainerID : string) => {
      ...
      const startFaceTracking = async () => {
        try {
          ...
          canvasContainer?.append(canvas);  // Optionally, append to a specific container
    
          console.log("what")
          const displaySize = { width: canvasContainer?.offsetWidth ?? 400, height: canvasContainer?.offsetHeight ?? 300 };
